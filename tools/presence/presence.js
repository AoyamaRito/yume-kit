// @why: Keep Why を「物理ブロック」ではなく「指摘」として扱う Presence チェッカー。
//       Git diff を検査し、コードが追加・変更されたのに @why 行が追加されていないファイルをオフラインで列挙する。
//       v1.4: これが失敗してもプロジェクトを止めない。あくまで「@why忘れ」のヒントであり、
//       直すかどうかは作業中のAI・人間が判断する（強制するのは物理的に壊れるものだけ）。
// @tags: SPEC
 ...
 * @param {string} [options.scope] スコープ指定（この配下のファイルのみ検証）。例: 'tenkai' や 'tenkai/lib'（リポジトリーを横断せず自分の変更分だけを見る）
...
export function checkWhyPresence(options = {}) {
  ...
  if (diff == null) {
    // 1) 未コミット（ステージ＋未ステージ）の差分を取得。scope があれば pathspec で自分の変更分だけに制限する。
    // @why: ワークスペースが複数プロジェクトを持つ場合、presenceを持断するデフォルトの git diff HEAD は他プロジェクトの
    //       未コミット差分まで拾い、自分の変更以外でFAILしていた。scope 指定で「自分の了承範囲」に限定できるようにする。
    // @tags: SPEC
    const pathspec = options.scope ? ['--', path.normalize(options.scope)] : [];
    diff = git(['diff', options.base || 'HEAD', ...pathspec], cwd) ?? '';
    if (diff === '') {
