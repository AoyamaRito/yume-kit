// @why: yume-spec の UI 健全性検証モジュール。E2Eテストでの reportUIHealthy 提供および pi の yui ツール実行ロジック
// @why: v1.4: 「不必要な強制をしない」方針により assertUIHealthy(throw) を主にせず、失敗しても止めない reportUIHealthy を
//       デフォルトにする。UIは壊れても E2E を落とす必要はなく、人間/AI が「指摘を見て直す」ヒントとして機能する。
// @tags: SPEC
...
/* ---------- グラフ抽出実行 ---------- */
// @why: [2026-09-04] 推奨アクション③「ブラウザ実動作の感覚器」。options.console が truthy のとき、console.error/warn・pageerror・requestfailed・4xx/5xx 応答を構造化ログとして収集する。E2E の前にブラウザ内部の実行時例外・通信失敗を機械的に検知する（read-only・ヒント。収集はしても throw しない）。
// @tags: SPEC
export async function runUIGraph(targetUrlOrPath, options = {}) {
  ...
  try {
    ...
    // @why: console 収集は goto 前にリスナーを張る（goto 中の console も拾う）。収集だけで絶対に throw しない（ヒント）
    const consoleIssues = [];
    const collectConsole = () => {
      ...
      page.on('console', (msg) => {
        ...
        if (type === 'error' || type === 'warning') {
          consoleIssues.push({ kind: 'console', type, text: msg.text().slice(0, 300) });
        }
      });
      page.on('pageerror', (err) => {
        consoleIssues.push({ kind: 'pageerror', type: 'error', text: String(err?.message || err).slice(0, 300) });
      });
      page.on('requestfailed', (req) => {
        consoleIssues.push({ kind: 'requestfailed', type: 'error', text: `${req.method()} ${req.url()} — ${req.failure()?.errorText || 'failed'}`.slice(0, 300) });
      });
      page.on('response', (res) => {
        const status = res.status();
        if (status >= 400) {
          consoleIssues.push({ kind: 'http', type: status >= 500 ? 'error' : 'warning', text: `${status} ${res.url()}`.slice(0, 300) });
        }
      });
    };
    collectConsole();
    ...
    if (options.console) result.console = consoleIssues;
    return result;
  } finally {
    await browser.close();
  }
...
// @why: UI健全性は「不合格でテストを止める強制」ではなく「指摘として報告する」ヒント。
//       デフォルトは reportUIHealthy（throwしない）。本当にテストを止めたい場合のみオプトインで assertUIHealthy を使う。
// @tags: SPEC
export async function reportUIHealthy(targetUrlOrPath, options = {}) {
  const res = await runUIGraph(targetUrlOrPath, options);
  res.report = renderAnomalies(res);
  return res;
}

// @why: 後方互換・明示的オプトイン用。本当に「UI検査でテストを止めたい」場合だけ使う（デフォルトは reportUIHealthy のヒント）
// @tags: SPEC
export async function assertUIHealthy(targetUrlOrPath, options = {}) {
  const res = await reportUIHealthy(targetUrlOrPath, options);
  if (!res.summary.pass) {
