...
export function extractUIGraph(options = {}) {
  ...
  function isInteractive(el, style) {
    ...
    // @why: 装飾SVGの内側rectはcursor:pointerを継承して「極小インタラクティブ素」と誤検出されていた。
    //       SVG描画子は絵であり操作対象でないため、明示的に非インタラクティブ扱いにする（ユーザー側にdata-yui-ignoreを
    //       撒く必要が無く、初期排除で誤検出を消す）。SVGルート自体(cursor:pointerなリンク内svg等)は残す。
    // @tags: SPEC
    if (el instanceof SVGElement && el.ownerSVGElement) return false; // SVGの描画子要素(rect/circle/path等)/絵
    if (tag === 'option') return false; // <option>は標準では未描画・ゼロビクセル（ゼロ値誤検出対策）
    if (['script', 'style', 'meta', 'title', 'head', 'noscript', 'template'].includes(tag)) return false;
    ...
    if (el.hasAttribute('role')) {
      ...
      if (role === 'presentation' || role === 'none') return false; // 装飾宣言は非インタラクティブ
    }
    if (style && style.pointerEvents === 'none') return false; // pointer-events:none は「非操作の装飾」宣言
  ...
  for (const el of rawElements) {
    ...
    // @why: 非表示祖先(display:none等)に内包される要素は個別のsecが0でも表示されない。
    //       isHiddenを「自身のstyle」だけではなく「祖先が非表示か」まで強くする（非表示バナー内含ボタンの
    //       ゼロ厚件・タップ誤報を抑止し、隠れ状態を探させる clearify に反しない）。
    // @tags: SPEC
    ...
    if (style.display === 'none' || style.visibility === 'hidden') {
      ...
      while (anc && anc !== document.body) {
        ...
        if (as.display === 'none' || as.visibility === 'hidden') { hiddenAncestor = true; break; }
        anc = anc.parentElement;
      }
    }
    const isHidden = hiddenAncestor || style.opacity === '0';
    ...
    if (!isHidden) {
      ...
      if (!isIgnored(el, 'ZERO_BOX_WITH_CONTENT')) {
        // @why: <option>は未描画のためゼロボックスが標準。isInteractiveがfalseなのでinteractive条件は効かないが、
        //       明示的にスキップする（<option>をUI破綻として検出するのはノイズであるため）。
        const isOptionEl = tag === 'option';
        if (!isOptionEl && (rect.width === 0 || rect.height === 0) && (directText.length > 0 || interactive || el.children.length > 0)) {
          const anomaly = {
      ...
      // 3. Overflow leak
      // @why: <option>(未描画)とSVG描画子(装飾絵)は isInteractive で非操作扱いだが、それでもOVERFLOW_LEAKで
      //       ゼロ許値残差を誤検出していた。絵・非描画は構造上の専有であり、装飾表示が意図されるのでoverFlowしない。
      // @tags: SPEC
      if (checkOverflow && el.parentElement && el.parentElement !== document.documentElement && !isIgnored(el, 'OVERFLOW_LEAK')
          && tag !== 'option' && !(el instanceof SVGElement && el.ownerSVGElement)) {
